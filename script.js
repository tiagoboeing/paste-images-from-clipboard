...
var CLIPBOARD = (function () {
   ...
   * @param callback
   */
  function convertFromClipboardToBlob(pasteEvent, callback) {
  ...
  function handleListener(emit) {
    return function (pasteEvent) {
      if (MODE === 'blob') {
        convertFromClipboardToBlob(pasteEvent, function (imageBlob) {
          if (imageBlob) {
            const canvas = document.getElementById('clipboard-canvas')
        ...
        //     // Handle the event
        retrieveImageFromClipboardAsBase64(pasteEvent, function (imageDataBase64) {
